...
import static de.terrarier.netlistening.util.LittleEndianByteArray.load32;
import static de.terrarier.netlistening.util.LittleEndianByteArray.load64;
 ...
 */
@ApiStatus.Internal
public final class FarmHashFingerprint64 implements HashFingerprint {
    ...
    public static long fingerprint(@AssumeNotNull byte[] bytes, int offset, int length) {
        if (length <= 32) {
            if (length <= 16) {
    ...
    private static long hashLength0to16(@AssumeNotNull byte[] bytes, int offset, int length) {
        if (length >= 8) {
    ...
    private static long hashLength17to32(@AssumeNotNull byte[] bytes, int offset, int length) {
        long mul = K2 + length * 2;
    ...
    private static long hashLength33To64(@AssumeNotNull byte[] bytes, int offset, int length) {
        long mul = K2 + length * 2;
        long a = load64(bytes, offset) * K2;
        long b = load64(bytes, offset + 8);
        long c = load64(bytes, offset + length - 8) * mul;
     ...
     */
    private static long hashLength65Plus(@AssumeNotNull byte[] bytes, int offset, int length) {
        final int seed = 81;
    ...
    // TODO: Document this!
    @Override
    public long fingerPrint(@AssumeNotNull byte[] data) {
