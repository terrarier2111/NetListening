...
public final class InternalPayloadUpdateTranslationEntry extends InternalPayload {
    ...
    @Override
    public void write(@NotNull ApplicationImpl application, @NotNull ByteBuf buffer) {
    ...
    public void read(@NotNull ApplicationImpl application, @NotNull ConnectionImpl connection, @NotNull ByteBuf buffer)
        ...
        if(application instanceof Server) {
            ...
            final ByteBuf translationUpdateBuffer = Unpooled.buffer(4 + 1 + 4); // TODO: Improve init size.
            new InternalPayloadUpdateTranslationEntry(id).write(application, buffer);
            connection.getChannel().writeAndFlush(translationUpdateBuffer);
        }
    }
